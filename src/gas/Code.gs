...
function doGet(e) {
  try {
    ...
    if (view === 'admin_pages') {
      return renderAdminPagesView(isEditor);
    }
    ...
    if (view === 'admin_audit') {
      return renderAdminAuditView(isEditor);
    }
 ...
 * @returns {HtmlOutput}
 */
function renderAdminPagesView(isEditor) {
  // Editorでない場合はアクセス拒否
  if (!isEditor) {
    return HtmlService.createHtmlOutput(
      '<html><body style="font-family: sans-serif; padding: 40px; text-align: center;">' +
 ...
 * @returns {HtmlOutput}
 */
function renderAdminAssetsView(isEditor) {
  // Editorでない場合はアクセス拒否
  if (!isEditor) {
    return HtmlService.createHtmlOutput(
      '<html><body style="font-family: sans-serif; padding: 40px; text-align: center;">' +
 ...
 * @returns {HtmlOutput}
 */
function renderAdminAuditView(isEditor) {
  // Editorでない場合はアクセス拒否
  if (!isEditor) {
    return HtmlService.createHtmlOutput(
      '<html><body style="font-family: sans-serif; padding: 40px; text-align: center;">' +
